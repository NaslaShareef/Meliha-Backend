...
namespace SalesForceAutomation.BO_Digits.en
{
    public partial class SalesDetails : System.Web.UI.Page
    {
        ...
        protected void btnPDF_Click(object sender, ImageClickEventArgs e)
        {
            if (ViewState["INVNumber"] != null)
            {
                try
                {
                    ...
                    ((StiSqlDatabase)report.Dictionary.Databases["MelihaReport"]).ConnectionString = DB;

                    report["@para2"] = ResponseID.ToString();   //ResponseID.ToString();
        ...
        protected void editlpo_Click(object sender, ImageClickEventArgs e)
        {
            txtLPONo.Text = "";
            DataTable dt= new DataTable();
            dt = ObjclsFrms.loadList("CurrentLPO", "sp_Merchandising_UOM", ResponseID.ToString());
            if (dt.Rows.Count > 0)
            {
                currentLpoNumber.Text =dt.Rows[0]["LPO"].ToString();
                ViewState["CurrentLPO"] = currentLpoNumber.Text;
            }
            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", "<script type='text/javascript'>Confim('Are you sure you want to update Invoice Details..??');</script>", false);

        }

        protected void updatelpo_Click(object sender, EventArgs e)
        {
            lblerror.Text = "";
            string lpo = txtLPONo.Text.Trim(); // Trim removes any leading or trailing spaces
            int res = 0;

            if (string.IsNullOrWhiteSpace(lpo))
            {
                lblerror.Text = "Please enter a valid LPO Number";
            ...
            else
            {
                ...
                try
                {
                    ...
                    string Resp = ObjclsFrms.SaveData("sp_Merchandising_UOM", "UpdateLPO", lpo, ar);
                ...
                if (res > 0)
                {
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", "<script type='text/javascript'>Succcess('LPO Number updated successfully');</script>", false);
