...
namespace SalesForceAutomation.BO_Digits.en
{
    public partial class MostPerformingDetail : System.Web.UI.Page
    {
        ...
        protected void imgExcel_Click(object sender, ImageClickEventArgs e)
        {
            ...
            foreach (GridColumn column in grvRpt2.MasterTableView.Columns)
            {
            ...
            DataRow dr;
            grvRpt2.MasterTableView.AllowPaging = false;

            RadGrid dd = grvRpt2;
            dd.AllowPaging = false;
            dd.Rebind();
            int x = grvRpt2.MasterTableView.Items.Count;
            foreach (GridDataItem item in dd.MasterTableView.Items)
            {
                dr = dt.NewRow();
                int j = 0;
                for (int i = 1; i < grvRpt2.MasterTableView.Columns.Count; i++)
                {
                    if (grvRpt2.MasterTableView.Columns[i].Display == true)
                    {
                        //if (i == 0)
                        //{
                        //    i++;
                        ...
                        if (!item[grvRpt2.MasterTableView.Columns[i].UniqueName].Text.Contains("Detail"))
                        {
                            if (!grvRpt2.MasterTableView.Columns[i].HeaderText.Equals("Image"))
                            {
                                dr[j] = item[grvRpt2.MasterTableView.Columns[i].UniqueName].Text;
                                j++;
                            }
            ...
            foreach (GridColumn column in grvRpt1.MasterTableView.Columns)
            {
                    ...
                    )
                {
                    ...
                    {
                        columncount++;
                        ds.Columns.Add(column.HeaderText.Replace("<br>", " "), typeof(string));
                    }
            ...
            RadGrid du = grvRpt1;
            du.AllowPaging = false;
            du.Rebind();
            int z = grvRpt1.MasterTableView.Items.Count;
            foreach (GridDataItem item in du.MasterTableView.Items)
            {
                dp = ds.NewRow();
                int j = 0;
                for (int i = 1; i < grvRpt1.MasterTableView.Columns.Count; i++)
                {
                    if (grvRpt1.MasterTableView.Columns[i].Display == true)
                    {
                        ...
                        if (!item[grvRpt1.MasterTableView.Columns[i].UniqueName].Text.Contains("Detail"))
                        {
                            ...
                            {
                                dp[j] = item[grvRpt1.MasterTableView.Columns[i].UniqueName].Text;
                                j++;
                            }
            ...
            SpreadStreamProcessingForXLSXAndCSV(dt,ds);
        }
        ...
        private void SpreadStreamProcessingForXLSXAndCSV(DataTable dt, DataTable ds, SpreadDocumentFormat docFormat = SpreadDocumentFormat.Xlsx, string sheetName = "AR")
        {
