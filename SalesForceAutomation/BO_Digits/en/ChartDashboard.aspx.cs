...
namespace SalesForceAutomation.BO_Digits.en
{
    public partial class ChartDashboard : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                try
                {
                    ...
                    MostPerformingOutlet(rotID, fromDate, ToDate);
        ...
        public void MostPerformingOutlet(string route, string fromDate, string ToDate)
        {
            string[] args = { fromDate, ToDate };
            DataTable dtMostPerformingOutlet = new DataTable();
            dtMostPerformingOutlet = ObjclsFrms.loadList("SelectMostPerformOutlet", "sp_Dashboard", route, args);
            string XAxis = "";
            string YAxis = "";
            string name = "";
            for (int i = 0; i < dtMostPerformingOutlet.Rows.Count; i++)
            {
                if (i < dtMostPerformingOutlet.Rows.Count - 1)
                {
                    XAxis += dtMostPerformingOutlet.Rows[i]["Code"].ToString() + "-";
                    YAxis += dtMostPerformingOutlet.Rows[i]["value_occurrence"].ToString() + "-";
                    name += dtMostPerformingOutlet.Rows[i]["Name"].ToString() + "{0}";
                }
                else if (i == dtMostPerformingOutlet.Rows.Count - 1)
                {
                    XAxis += dtMostPerformingOutlet.Rows[i]["Code"].ToString();
                    YAxis += dtMostPerformingOutlet.Rows[i]["value_occurrence"].ToString();
                    name += dtMostPerformingOutlet.Rows[i]["Name"].ToString();
                }
            }
            string OutletBarChart = "ApplyVertBarChartPopularOutlet('" + XAxis.ToString() + "' , '" + YAxis.ToString() + "', '" + name.ToString() + "');";
            ViewState["Chart"] += OutletBarChart;
        }
